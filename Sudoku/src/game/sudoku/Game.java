...
public class Game extends Activity {
	...
	protected void onResume() {
		super.onResume();
		Music.play(this, R.raw.drakebottom);
	}
	...
	boolean setTileIfValid(int x, int y, int value) {
		if(initPuzzle[y*9+x]>0) return false;//you can't change the initial numbers
		int tiles[] = getUsedTiles(x, y);
		if (value != 0) {
			for (int tile : tiles) {
				if (tile == value)
	...
	int[] calculateUsedTiles(int x, int y) {
		int c[] = new int[9];
		// horizontal
		for (int i = 0; i < 9; i++) {
			if (i == y)
