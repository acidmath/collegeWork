...
// lib/impact/timer.js
ig.baked=true;ig.module('impact.timer').defines(function(){ig.Timer=ig.Class.extend({target:0,base:0,last:0,pause:false,init:function(seconds){this.base=ig.Timer.time;this.last=ig.Timer.time;this.target=seconds||0;},set:function(seconds){this.target=seconds||0;this.base=ig.Timer.time;},reset:function(){this.base=ig.Timer.time;},tick:function(){if(!this.pause){var delta=ig.Timer.time-this.last;this.last=ig.Timer.time;return delta;}
else{this.last=ig.Timer.pauseTime;return 0;}},delta:function(){return ig.Timer.time-this.base-this.target;}});ig.Timer._last=0;ig.Timer.time=0;ig.Timer.pauseTime=0;ig.Timer.timeScale=1;ig.Timer.maxStep=0.05;ig.Timer.step=function(){var current=Date.now();var delta=(current-ig.Timer._last)/1000;var plusTime=Math.min(delta,ig.Timer.maxStep)*ig.Timer.timeScale;if(!this.pause){if(ig.Timer.time!=ig.Timer.pauseTime)
ig.Timer.pauseTime=ig.Timer.time;ig.Timer.time+=plusTime;}
ig.Timer.pauseTime+=plusTime;ig.Timer._last=current;};});

// lib/impact/system.js
ig.baked=true;ig.module('impact.system').requires('impact.timer','impact.image').defines(function(){ig.System=ig.Class.extend({fps:30,width:320,height:240,realWidth:320,realHeight:240,scale:1,tick:0,intervalId:0,newGameClass:null,running:false,delegate:null,clock:null,canvas:null,context:null,pause:false,smoothPositioning:true,init:function(canvasId,fps,width,height,scale){this.fps=fps;this.clock=new ig.Timer();this.canvas=ig.$(canvasId);this.resize(width,height,scale);this.context=this.canvas.getContext('2d');},resize:function(width,height,scale){this.width=width;this.height=height;this.scale=scale||this.scale;this.realWidth=this.width*this.scale;this.realHeight=this.height*this.scale;this.canvas.width=this.realWidth;this.canvas.height=this.realHeight;},setGame:function(gameClass){if(this.running){this.newGameClass=gameClass;}
else{this.setGameNow(gameClass);}},setGameNow:function(gameClass){ig.game=new(gameClass)();ig.system.setDelegate(ig.game);},setDelegate:function(object){if(typeof(object.run)=='function'){this.delegate=object;this.startRunLoop();}else{throw('System.setDelegate: No run() function in object');}},stopRunLoop:function(){clearInterval(this.intervalId);this.running=false;},startRunLoop:function(){this.stopRunLoop();this.intervalId=setInterval(this.run.bind(this),1000/this.fps);this.running=true;},clear:function(color){this.context.fillStyle=color;this.context.fillRect(0,0,this.realWidth,this.realHeight);},run:function(){if(!this.pause){ig.Timer.step();}
this.tick=this.clock.tick();this.delegate.run();ig.input.clearPressed();if(this.newGameClass){this.setGameNow(this.newGameClass);this.newGameClass=null;}},getDrawPos:function(p){return this.smoothPositioning?Math.round(p*this.scale):Math.round(p)*this.scale;}});});
...
this.updateEntities();this.checkEntities();for(var i=0;i<this._deferredKill.length;i++){var killedEnt=this._deferredKill[i];this.entities.erase(killedEnt);this.form.totalEnemyArr.erase(killedEnt);}
...
// lib/game/entities/galaxy.js
ig.baked=true;ig.module('game.entities.galaxy').requires('impact.entity').defines(function(){Galaxy=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/galaxy64.png',64,64),maxVel:{x:150,y:250},zIndex:-1,init:function(x,y,settings){this.addAnim('idle',0.1,[0]);this.parent(x,y,settings);var vel=Math.floor((Math.random()*35+15));this.vel.y=vel;},update:function(){this.parent();if(this.pos.y>ig.system.height+64){this.kill();}
this.currentAnim.angle+=ig.system.tick/1.4;}})});

// lib/game/entities/player.js
ig.baked=true;ig.module('game.entities.player').requires('impact.entity').defines(function(){EntityPlayer=ig.Entity.extend({size:{x:48,y:48},maxVel:{x:450,y:0},animSheet:new ig.AnimationSheet('media/space mailman-transparency.png',48,48),weapon:[.35,0],type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,invince:false,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0,1,2,3,2,1]);this.addAnim('invince',0.1,[0,4]);if(ig.game.lives<3)
this.currentAnim=this.anims.invince;},update:function(){this.parent();if(this.anims.invince.loopCount==15)
this.currentAnim=this.anims.idle;this.vel.x=0;this.weapon[1]+=ig.system.tick;if(ig.input.state('left')){if(this.pos.x>1)
this.vel.x-=275}
if(ig.input.state('right')){if(this.pos.x<ig.system.width-this.size.x-1)
this.vel.x+=275;}
if(ig.input.state('fire')){if(this.weapon[1]>this.weapon[0]){this.fire();this.weapon[1]=0.0;}}
if(ig.input.state('sfire')){if(this.weapon[1]>this.weapon[0]){this.weapon[1]=0.0;this.dshot.play();ig.game.spawnEntity(SpreadShotLeft,this.pos.x+12,this.pos.y+4,settings);ig.game.spawnEntity(SpreadShot,this.pos.x+24,this.pos.y+4,settings);ig.game.spawnEntity(SpreadShotRight,this.pos.x+36,this.pos.y+4,settings);}}},fire:function(){var shot;switch(ig.game.weaponType){case 0:for(var i=0;i<=1;i++){var pos=9;if(i){pos=36;}
var settings={flip:i};shot=ig.game.spawnEntity(DoubleShot,this.pos.x+pos,this.pos.y+4,settings);}
break;case 1:for(var j=-1;j<2;j++){var velX=j*100;var posX=(24+j*12)+this.pos.x;var posY=this.pos.y+4;var settings={vel:{x:velX,y:-500},pos:{x:posX,y:posY}};shot=ig.game.spawnEntity(SpreadShot,0,0,settings);}
break;case 2:shot=ig.game.spawnEntity(ApShot,this.pos.x+20,this.pos.y+4,settings);break;}
if(ig.game.mute)shot.sound.play();},check:function(other){ig.game.spawnEntity(Explosion,this.pos.x,this.pos.y);if(this.currentAnim!=this.anims.invince)
this.kill();other.kill();},kill:function(){this.parent();ig.game.lives--;if(ig.game.lives>0){ig.game.player=ig.game.spawnEntity(EntityPlayer,this.pos.x,this.pos.y);}
else{ig.game.menu=true;}}});});

// lib/game/entities/pointer.js
ig.baked=true;ig.module('game.entities.pointer').requires('impact.entity').defines(function(){Pointer=ig.Entity.extend({type:4,size:{x:1,y:1},init:function(x,y,settings){this.parent(x,y,settings);},update:function(){this.pos.x=ig.input.mouse.x;this.pos.y=ig.input.mouse.y;this.parent();}});});

// lib/game/entities/star.js
ig.baked=true;ig.module('game.entities.star').requires('impact.entity').defines(function(){Star=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/Star.png',5,5),maxVel:{x:150,y:350},zIndex:-1,init:function(x,y,settings){this.addAnim('large',0.18,[0,1]);this.addAnim('small',0.18,[2,3]);this.parent(x,y,settings);var vel=Math.floor((Math.random()*70+130));if(vel<=155){this.currentAnim=this.anims.small.rewind();}
this.vel.y=vel;},update:function(){this.parent();if(this.pos.y>ig.system.height+64){this.kill();}}})});

// lib/game/entities/enemy.js
ig.baked=true;ig.module('game.entities.enemy').requires('impact.entity').defines(function(){Enemy=ig.Entity.extend({type:'8',checkAgainst:ig.Entity.TYPE.B,size:{x:16,y:20},animSheet:new ig.AnimationSheet('media/mailbox2.png',16,20),maxVel:{x:450,y:450},speedF:1,zigs:[],fnum:null,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('open',0.2,[0,1,2],true);this.addAnim('close',0.1,[2,1,0],true);this.anims.open.flip.y=1;this.anims.close.flip.y=1;this.vel.y=150+this.speedF;this.fnum=settings;},check:function(other){ig.game.spawnEntity(Explosion,this.pos.x,this.pos.y);this.kill();ig.game.score+=20;if(other.currentAnim!=other.anims.invince)
other.receiveDamage(1);},update:function(){this.parent();if(this.anims.open.loopCount){this.anims.open.rewind();this.currentAnim=this.anims.close.rewind();ig.game.spawnEntity(EnemyShot,this.pos.x+this.size.x/2,this.pos.y+this.size.y/2);}
if(this.anims.close.loopCount){this.anims.close.rewind();this.currentAnim=this.anims.open.rewind();}
if(this.pos.y>ig.system.height+5){ig.game.form.updateStats(this.fnum);this.kill();}
if(this.pos.y>=-20){this.type=ig.Entity.TYPE.A;}}})});

// lib/impact/formation.js
ig.baked=true;ig.module('impact.formation').requires('impact.entity','impact.game','impact.impact').defines(function(){ig.Formation=ig.Class.extend({formTimer:null,enemies:0,enemyArr:null,dummy:null,form:1,delta:0,pi:3.14,totalEnemyArr:[],pause:false,stats:[],init:function()
{this.enemies=0;this.enemyArr=[];this.stats[3]=100;this.stats[1]=100;this.stats[2]=100;},spawnFormation:function(f)
{var xInit=Math.random()*(ig.system.width-100)+100;if(f==3)
{var s=ig.game.spawnEntity(Enemy,xInit,-48,3);this.enemyArr[0]=s;this.totalEnemyArr=[];for(var i=0;i<this.enemies;i++)
{var t=ig.game.spawnEntity(Enemy,xInit,-48,3);this.enemyArr[i]=t;this.totalEnemyArr.push(t);}}
if(f==1)
{var t=ig.game.spawnEntity(Enemy,xInit,-48,1);this.enemyArr[0]=t;this.totalEnemyArr=[];for(var i=1;i<this.enemies;i++)
{var s=ig.game.spawnEntity(Enemy,xInit,-48*i,1);this.enemyArr[i]=s;this.totalEnemyArr.push(s);}}
if(f==2){var t=ig.game.spawnEntity(Enemy,xInit,-64-196,2);this.dummy=ig.game.spawnEntity(Dummy,xInit,-64-196,2);this.enemyArr[0]=t;this.totalEnemyArr=[];for(var i=1;i<this.enemies;i++){var s=ig.game.spawnEntity(Enemy,xInit,24*Math.sin(i*2*this.pi/this.enemies-196),2);this.enemyArr[i]=s;this.totalEnemyArr.push(s);}}},updateStats:function(f)
{this.stats[f]+=2;console.log("formation "+f+" increased to "+this.stats[f]);},update:function(delta){if(!ig.game.pause){if(delta>5)
{if(this.enemies<10)this.enemies+=1;var f=Math.random()*(this.stats[3]+this.stats[1]+this.stats[2]);if(f<this.stats[3])this.form=3;else if(f<(this.stats[1]+this.stats[3]))this.form=1;else this.form=2;console.log("f: "+f+" this.form: "+this.form+"\n");this.spawnFormation(this.form);return true;}
else{if(this.form==3)
...
if(this.form==1)
{for(var i=0;i<this.enemies;i++)
{this.enemyArr[i].pos.x=this.enemyArr[i].pos.x+(this.enemyArr[i].vel.y/50)*Math.cos(delta*2.094*4);this.enemyArr[i].pos.y=this.enemyArr[i].pos.y+(this.enemyArr[i].vel.y/50)*Math.sin(delta*2.094*4);}}
if(this.form==2)
{for(var i=1;i<this.enemies;i++)
{this.enemyArr[i].pos.x=this.dummy.pos.x+48*Math.cos(delta*5+i*(2*this.pi/(this.enemies-1)));this.enemyArr[i].pos.y=this.dummy.pos.y+48*Math.sin(delta*5+i*(2*this.pi/(this.enemies-1)));}}
return false;}}else{return false;}}});});

// lib/game/entities/dummy.js
ig.baked=true;ig.module('game.entities.dummy').requires('impact.entity','impact.formation').defines(function(){Dummy=ig.Entity.extend({type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,size:{x:16,y:20},maxVel:{x:450,y:450},speedF:1,init:function(x,y,settings){this.parent(x,y,settings);this.vel.y=151;},update:function()
{this.parent();}});});

// lib/game/entities/doubleshot.js
ig.baked=true;ig.module('game.entities.doubleshot').requires('impact.entity').defines(function(){DoubleShot=ig.Entity.extend({size:{x:4,y:8},maxVel:{x:200,y:400},animSheet:new ig.AnimationSheet('media/bullet.png',4,8),type:ig.Entity.TYPE.B,health:1,flip:0,sound:new ig.Sound('media/sounds/dsSound.*'),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.currentAnim.flip.x=this.flip;this.vel.y=-400;},update:function(){this.parent();if(this.pos.y<5){this.kill();}}});});

// lib/game/entities/spreadshot.js
ig.baked=true;ig.module('game.entities.spreadshot').requires('impact.entity').defines(function(){SpreadShot=ig.Entity.extend({size:{x:2,y:4},maxVel:{x:200,y:400},animSheet:new ig.AnimationSheet('media/spreadbullet.png',2,4),collision:ig.Entity.COLLIDES.ACTIVE,type:ig.Entity.TYPE.B,health:1,flip:0,sound:new ig.Sound('media/sounds/dsSound.*'),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);},update:function(){this.parent();if(this.pos.y<5){this.kill();}}});SpreadShotLeft=ig.Entity.extend({size:{x:2,y:4},maxVel:{x:200,y:400},animSheet:new ig.AnimationSheet('media/spreadbullet.png',2,4),collision:ig.Entity.COLLIDES.ACTIVE,type:ig.Entity.TYPE.B,health:1,flip:0,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.currentAnim.flip.x=this.flip;this.vel.y=-500;this.vel.x=-100;},update:function(){this.parent();if(this.pos.y<5){this.kill();}}});SpreadShotRight=ig.Entity.extend({size:{x:2,y:4},maxVel:{x:200,y:400},animSheet:new ig.AnimationSheet('media/spreadbullet.png',2,4),collision:ig.Entity.COLLIDES.ACTIVE,type:ig.Entity.TYPE.B,health:1,flip:0,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.currentAnim.flip.x=this.flip;this.vel.y=-500;this.vel.x=100;},update:function(){this.parent();if(this.pos.y<5){this.kill();}}});});

// lib/game/entities/apshot.js
ig.baked=true;ig.module('game.entities.apshot').requires('impact.entity').defines(function(){ApShot=ig.Entity.extend({size:{x:8,y:12},maxVel:{x:200,y:400},animSheet:new ig.AnimationSheet('media/ap-bullet.png',8,12),type:ig.Entity.TYPE.B,health:100,flip:0,sound:new ig.Sound('media/sounds/dsSound.*'),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.currentAnim.flip.x=this.flip;this.vel.y=-400;},update:function(){this.parent();if(this.pos.y<5){this.kill();}}});});

// lib/game/entities/explosion.js
ig.baked=true;ig.module('game.entities.explosion').requires('impact.entity').defines(function(){Explosion=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/explosion.png',32,32),explodeSound:new ig.Sound('media/sounds/bomb.*'),init:function(x,y,settings){this.parent(x,y,settings);this.explodeSound.volume=0.2;this.addAnim('idle',0.13,[1,2,3,4,5,6,7],true);if(ig.game.mute)
this.explodeSound.play();},update:function(){this.parent();if(this.currentAnim.loopCount)
this.kill();}})});

// lib/game/entities/enemyshot.js
ig.baked=true;ig.module('game.entities.enemyshot').requires('impact.entity').defines(function(){EnemyShot=ig.Entity.extend({type:ig.Entity.TYPE.A,size:{x:9,y:14},maxVel:{x:200,y:400},animSheet:new ig.AnimationSheet('media/letter2.png',9,14),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.vel.y=300;},update:function(){this.parent();if(this.pos.y>ig.system.height+5){this.kill();}}});});

// lib/game/entities/menu.js
ig.baked=true;ig.module('game.entities.menu').requires('impact.entity').defines(function(){Menu=ig.Entity.extend({checkAgainst:4,size:{x:480,y:20},message:null,option:0,ref:{x:0,y:0},init:function(x,y,settings){this.parent(x,y,settings);switch(this.option){case 0:this.message="Resume";break;case 1:this.message="Restart";break;case 2:this.message="Main Menu";break;}},check:function(){if(ig.input.pressed('click')){switch(this.option){case 0:ig.main.unpause(ig.game);break;case 1:ig.game.start();break;case 2:ig.music.stop();ig.main.unpause(ig.game);ig.system.setGame(MyGame);break;}}},draw:function(){this.parent();ig.game.font.draw(this.message,this.ref.x,this.ref.y,ig.Font.ALIGN.CENTER);}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','game.entities.galaxy','impact.font','game.entities.player','game.entities.pointer','game.entities.star','game.entities.enemy','game.entities.dummy','game.entities.doubleshot','game.entities.spreadshot','game.entities.apshot','game.entities.explosion','game.entities.enemyshot','game.entities.menu','impact.formation').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/font.png'),normalShot:null,spreadShot:null,apShot:null,liveImage:null,enemyTimer:null,starTimer:null,playTime:new ig.Timer(0),bHud:null,BH:null,menu:true,mute:true,formTimer:null,form:null,weaponType:0,pause:false,pointer:null,nShot:null,sShot:null,aShot:null,liveStock:null,lives:3,score:0,menus:[],init:function(){ig.music.add('media/sounds/messinchip2.*');ig.music.volume=0.5;ig.music.play();ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.SPACE,'fire');ig.input.bind(ig.KEY.ENTER,'start');ig.input.bind(ig.KEY.M,'mute');ig.input.bind(ig.KEY.P,'pause');ig.input.bind(ig.KEY.MOUSE1,'click');this.pointer=this.spawnEntity(Pointer,0,0);this.bHud=new ig.AnimationSheet('media/hud.png',50,50);this.hud=new ig.Animation(ig.game.bHud,0.1,[0]);this.normalShot=new ig.AnimationSheet('media/Nbullet.png',50,50);this.spreadShot=new ig.AnimationSheet('media/Sbullet.png',50,50);this.apShot=new ig.AnimationSheet('media/APbullet.png',50,50);this.liveImage=new ig.AnimationSheet('media/space mailman-icon.png',24,24);this.nShot=new ig.Animation(ig.game.normalShot,0.1,[0]);this.sShot=new ig.Animation(ig.game.spreadShot,0.1,[0]);this.aShot=new ig.Animation(ig.game.apShot,0.1,[0]);this.liveStock=new ig.Animation(ig.game.liveImage,0.1,[0]);},update:function(){this.parent();this.keyboardCheck();if(!this.menu){if(!this.pause){this.score+=ig.system.tick*10;this.weaponSwitch();this.spawnStar();if(this.form.update(this.formTimer.delta()))this.formTimer.set(0);}}},keyboardCheck:function(){if(ig.input.pressed('mute')){if(this.mute){this.mute=false;ig.music.pause();}else{this.mute=true;ig.music.play();}}
var ent;if(ig.input.pressed('pause')&&!this.menu){if(this.pause)
ig.main.unpause(this);else
ig.main.pauseNow(this);}
if(ig.input.pressed('start')){if(this.menu){this.menu=false;this.start();}}},spawnStar:function(){if(this.starTimer.delta()>0){ig.game.sortEntitiesDeferred();var x=Math.ceil(Math.floor(Math.random()*ig.system.width));this.spawnEntity(Star,x,-48);this.starTimer.set(.08);}},weaponSwitch:function(){if(ig.input.pressed('up')){this.weaponType+=1;if(this.weaponType==3){this.weaponType=0;}}
if(ig.input.pressed('down')){this.weaponType-=1;if(this.weaponType==-1){this.weaponType=2;}}},draw:function(){this.parent();if(this.menu){this.font.draw('Press Enter to Start',ig.system.width/2,ig.system.height/2,ig.Font.ALIGN.CENTER);if(this.score>0)
this.font.draw('Score: '+~~this.score,ig.system.width/2,ig.system.height/2+22,ig.Font.ALIGN.CENTER);}
if(!this.menu){this.font.draw("Score: ",ig.system.width-50,0,ig.Font.ALIGN.RIGHT);this.font.draw(~~this.score+"",ig.system.width,0,ig.Font.ALIGN.RIGHT);this.showInstructions(this.pause);this.showHUD(!this.pause);}},showHUD:function(notInMenu){if(notInMenu){var y;var x=ig.system.width-30;for(var i=this.lives;i>0;i--){y=ig.system.height-30;this.liveStock.draw(x,y);x-=24;}
this.hud.draw(0,ig.system.height*(11/12));switch(ig.game.weaponType){case 0:this.nShot.draw(0,ig.system.height*(11/12));break;case 1:this.sShot.draw(0,ig.system.height*(11/12));break;case 2:this.aShot.draw(0,ig.system.height*(11/12));break;}}},showInstructions:function(isPaused){if(isPaused){var y=0;var x=4;this.font.draw("Instructions",x,y,ig.Font.ALIGN.LEFT);this.font.draw("------------",x,y+11,ig.Font.ALIGN.LEFT);this.font.draw("Spacebar: Fire",x,y+33,ig.Font.ALIGN.LEFT);this.font.draw("Left/Right Arrows: Move Left/Right",x,y+55,ig.Font.ALIGN.LEFT);this.font.draw("Up/Down Arrows: Switch Weapon",x,y+77,ig.Font.ALIGN.LEFT);this.font.draw("P: Pause",x,y+99,ig.Font.ALIGN.LEFT);this.font.draw('M: Mute Sound',x,y+121,ig.Font.ALIGN.LEFT);}},start:function(){for(var i=0;i<this.entities.length;i++){this.entities[i].kill();}
this.menu=false;this.weaponType=0;this.lives=3;this.score=0;this.pointer=this.spawnEntity(Pointer,0,0);var random=Math.ceil((Math.random()*3+1.75));this.enemyTimer=new ig.Timer(random);random=(Math.random()*1.8+.25);this.starTimer=new ig.Timer(random);this.formTimer=new ig.Timer(8);this.player=this.spawnEntity(EntityPlayer,ig.system.width/2-24,ig.system.height*(4/5));this.form=new ig.Formation();ig.main.unpause(this);}});ig.main('#canvas',MyGame,60,480,600,1);ig.main.unpause=function(game){ig.system.clock.pause=false;ig.system.pause=false;game.pause=false;for(var k=0;k<game.menus.length;k++){game.menus[k].kill();}
game.menus=[];for(var j=0;j<game.entities.length;j++){ent=game.entities[j];ent.pause=false;game.formTimer.pause=false;if(ent.currentAnim!=null){ent.currentAnim.timer.pause=false;ent.currentAnim.alpha=1;}}};ig.main.pauseNow=function(game){game.pause=true;ig.system.clock.pause=true;game.formTimer.pause=true;ig.system.pause=true;var wMid=ig.system.width/2;var hMid=ig.system.height/2-22*2;var settings;for(var l=0;l<3;l++){var height=hMid+l*22;settings={option:l,ref:{x:wMid,y:height}};game.menus.push(game.spawnEntity(Menu,0,height,settings));}
for(var i=0;i<game.entities.length;i++){ent=game.entities[i];ent.pause=true;if(ent.currentAnim!=null){ent.currentAnim.timer.pause=true;ent.currentAnim.alpha=0.45;}}};ig.main.restart=function(game){};});